...
export default function ProfileMenu() {
    ...
    return (
        <div className="relative" ref={ref}>
            ...
            {open && (
                <div className="absolute left-0 top-11 z-50 w-44 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-lg">
                    <div className="border-b border-slate-100 px-3 py-2">
                        <p className="truncate text-xs font-semibold text-slate-800">{name}</p>
                    </div>
                    <button onClick={goProfile} className="block w-full px-3 py-2 text-left text-sm text-slate-700 hover:bg-slate-50">
                        Profile
                    </button>
