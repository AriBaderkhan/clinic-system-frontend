...
const INPUT_CLS = "w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-900 outline-none focus:border-[#0E6E75] focus:bg-white focus:ring-1 focus:ring-[#0E6E75]";
  ...
  const print = () => {
    ...
    const html = `
      ...
      <style>
        ...
        h1{color:#0E6E75;margin:0 0 4px}
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <button type="button" onClick={print}
          className="rounded-lg border border-[#0E6E75] px-3 py-1.5 text-xs font-medium text-[#0E6E75] transition hover:bg-[#0E6E75] hover:text-white">
          {t("rx.print")}
        </button>
      </div>
      ...
      {items.length === 0 ? (
        ...
        <div className="space-y-2">
          {items.map((row, idx) => (
            <div key={idx} className={`flex items-start justify-between gap-2 rounded-lg border px-3 py-2 ${editingIdx === idx ? "border-[#0E6E75] bg-[#0E6E75]/5" : "border-slate-200 bg-white"}`}>
              ...
              {!readOnly && (
                <div className="flex shrink-0 items-center gap-1">
                  <button type="button" onClick={() => editRow(idx)} title={t("common.edit")}
                    className="rounded p-1 text-slate-400 hover:bg-slate-100 hover:text-[#0E6E75]">✎</button>
      ...
      {!readOnly && (showForm ? (
        <div className="space-y-2 rounded-xl border border-slate-200 bg-slate-50/60 p-3">
          ...
          <div className="flex items-center justify-end gap-2">
            ...
            <button type="button" onClick={addOrSave}
              className="rounded-lg bg-[#0E6E75] px-4 py-1.5 text-xs font-semibold text-white hover:bg-[#0A565C]">
        ...
        <button type="button" onClick={openAdd}
          className="w-full rounded-lg border border-dashed border-slate-300 px-3 py-2 text-xs font-medium text-slate-600 hover:border-[#0E6E75] hover:text-[#0E6E75]">
