      ...
      <div className="flex max-h-[96vh] w-full max-w-7xl flex-col rounded-2xl bg-white shadow-xl border border-slate-200">
        ...
        <div className="flex items-start justify-between border-b border-slate-100 px-4 sm:px-5 py-3 shrink-0">
          <div>
            ...
            <p className="mt-0.5 text-[11px] text-slate-500">{t("sd.subtitle_docs")}</p>
          </div>
          <div className="flex items-center gap-2">
            <button type="button" onClick={() => setEditing(true)} className="rounded-md bg-[#015478] px-3 py-1.5 text-[11px] font-medium text-white hover:bg-[#013d58]">{t("sd.edit_works_images")}</button>
        ...
        {isLoading ? (
          ...
          <div className="min-h-0 flex-1 overflow-y-auto p-3 sm:p-4 space-y-3">
            ...
            {/* Middle: details + works  |  teeth chart */}
            <div className="grid gap-3 lg:grid-cols-[minmax(300px,0.85fr),2fr] lg:items-start">
              <div className="space-y-3">
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-3">
                  <div className="mb-2 flex items-center justify-between">
                    ...
                    <span className="rounded-full bg-[#015478]/10 px-3 py-1 text-[11px] font-medium text-[#015478] border border-[#015478]/20">{t("sd.items_count", { count: worksSummary?.works?.length || 0 })}</span>
                  </div>
                  {!hasWorks ? (
                      ...
                      <table className="min-w-full text-left text-sm">
                        <thead>
                          <tr className="border-b border-slate-200 text-xs text-slate-500">
                            <th className="px-3 py-2 font-medium text-[#015478]">{t("sd.col_work")}</th>
                            <th className="px-3 py-2 font-medium text-[#015478]">{t("sd.col_qty")}</th>
                            <th className="px-3 py-2 font-medium text-[#015478]">{t("sd.col_teeth")}</th>
                          </tr>
                        </thead>
                        <tbody>
              ...
              <div className="flex flex-col rounded-xl border border-[#015478]/20 bg-[#015478]/5 p-3 sm:p-4">
                <div className="mb-2 flex flex-wrap items-start justify-between gap-2">
                  <p className="text-sm font-semibold text-[#015478] dark:text-sky-300">{t("sd.teeth_chart")}</p>
